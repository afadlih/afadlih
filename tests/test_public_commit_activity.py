...
class PublicCommitActivityTests(unittest.TestCase):
    def setUp(self):
        ...
        self.fixture = json.loads((ROOT / "tests" / "fixtures" / "public-commits-api.json").read_text(encoding="utf-8"))
        self.private_activity = json.loads((ROOT / "portfolio" / "profile.json").read_text(encoding="utf-8"))["private_activity"]

    def test_snapshot_uses_fixed_thirty_day_window(self):
        snapshot = update.build_snapshot(self.sources, "afadlih", fixture=self.fixture)
        ...
        self.assertEqual("afadlih/afadlih", snapshot["repositories"][0]["repository"])

    def test_incomplete_refresh_keeps_previous_complete_snapshot(self):
        previous = update.build_snapshot(self.sources, "afadlih", fixture=self.fixture)
        incomplete = json.loads(json.dumps(self.fixture))
        del incomplete["commits"]["afadlih/2341720069_ML_2025"]
        result = update.build_snapshot(self.sources, "afadlih", fixture=incomplete, previous=previous)
        self.assertEqual(previous, result)

    def test_writer_and_renderer_are_deterministic(self):
        snapshot = update.build_snapshot(self.sources, "afadlih", fixture=self.fixture)
        svg = render.render_svg(snapshot, self.private_activity)
        self.assertIn("ENGINEERING ACTIVITY", svg)
        self.assertIn("PRIVATE WORK SNAPSHOT", svg)
        self.assertIn("AquaSense", svg)
        self.assertIn("no URLs, branches, SHAs, or commit messages", svg)
        self.assertNotIn("github.com/afadlih/AquaSense", svg)
